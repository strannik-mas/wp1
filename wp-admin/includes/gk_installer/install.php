...
function gk_installer_run($table_prefix, $wpdb) {
	...
	$options_dump = str_replace(array('{$table_prefix}', '{$page_url}'), array($table_prefix, $page_url), $options_dump);
	...
	$wpdb->query('ALTER TABLE '.$table_prefix.'comments AUTO_INCREMENT=34;');
	$wpdb->query($options_dump);
	$wpdb->query('ALTER TABLE '.$table_prefix.'options AUTO_INCREMENT=8396;');
	$wpdb->query($postmeta_dump);
	$wpdb->query('ALTER TABLE '.$table_prefix.'postmeta AUTO_INCREMENT=1328;');
	$wpdb->query($posts_dump);
	$wpdb->query('ALTER TABLE '.$table_prefix.'posts AUTO_INCREMENT=817;');
	$wpdb->query($term_rel_dump);
	// no alter table for the term_relationships
	$wpdb->query($term_tax_dump);
	$wpdb->query('ALTER TABLE '.$table_prefix.'term_taxonomy AUTO_INCREMENT=85;');
	$wpdb->query($terms_dump);
	$wpdb->query('ALTER TABLE '.$table_prefix.'terms AUTO_INCREMENT=86;');
}

function gk_installer_predefined_options($wpdb, $table_prefix) {
	// set the theme
	update_option('template', "MeetGavernWP");
	update_option('stylesheet', "MeetGavernWP");
}
